import { Timeline, Animation } from '../lib/animation/animation'
import { cubicBezier, ease } from '../lib/animation/cubicBezier'
...
export class Carousel {
  ...
  render() {
    ...
    let children = this.data.map((url, currentPosition) => {
      ...
      let onPanend = event => {
        let direction = 0
        const { clientX, startX, isFlick } = event.detail
        // 手势偏移值
        let dx = clientX - startX

        console.log('flick', isFlick)

        if (dx + offset > 250 || (dx > 0 && isFlick)) {
          direction = 1
        } else if (dx < -250 || (dx < 0 && isFlick)) {
          direction = -1
        }
